...
// Part 1
let totalCountOfAnsweredQuestions = 0;
groups.forEach((group) => {
  group = group.replaceAll("\n", "");
  group = group.split("");
  const uniqueAnsweredQuestions = [...new Set(group)];
  totalCountOfAnsweredQuestions += uniqueAnsweredQuestions.length;
});

// console.log(totalCountOfAnsweredQuestions);

// Part 2
let totalCountOfSamenAnsweredQuestions = 0;
groups.forEach((group) => {
  const allLetters = group.replaceAll("\n", "");
  const people = group.split("\n");
  const amountOfPeople = people.length;
  const answeredQuestionsOfFirstPerson = people[0].split("");

  let sameAnswersAsOtherPeople = 0;
  answeredQuestionsOfFirstPerson.forEach((letter) => {
    const timesLetterAppearsInAllLetters = allLetters.split(letter).length - 1;
    if (timesLetterAppearsInAllLetters === amountOfPeople) {
      sameAnswersAsOtherPeople++;
    }
  });

  totalCountOfSamenAnsweredQuestions += sameAnswersAsOtherPeople;
});
console.log(totalCountOfSamenAnsweredQuestions);
