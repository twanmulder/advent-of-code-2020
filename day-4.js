...
for (var i = 0; i < lines.length; i++) {
  ...
  if (semiColonCount === 7 && !line.includes("cid")) {
    validPassports++;
  }
  if (semiColonCount === 8) {
    validPassports++;
...
// Part 2
function checkIfValueIsValid(key, value) {
  if (key === "byr") {
...
for (var i = 0; i < lines.length; i++) {
  ...
  pairs.forEach((pair) => {
    ...
    const isValid = checkIfValueIsValid(key, value);
    if (isValid) {
      validPairs++;
    }
  });

  if (validPairs === 7 && !line.includes("cid")) {
    validPassports++;
  }
