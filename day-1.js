...
function solveForFirstStar(input) {
  ...
  for (var i = 0; array.length - 1 > i; i++) {
    ...
    for (var j = i + 1; array.length > j; j++) {
      var secondNumber = array[j];

      for (var k = j + 1; array.length > k; k++) {
...
function solveForSecondStar(input) {
  ...
  for (var i = 0; array.length - 1 > i; i++) {
    for (var j = 0; array.length > j; j++) {
      var addedNumbers = array[i] + array[j];
      if (addedNumbers < 2020) {
        firstArray.push(addedNumbers);
        secondArray.push(array[i] * array[j]);
      }
    }
  }

  for (var k = 0; firstArray.length > k; k++) {
    for (var l = 0; array.length > l; l++) {
      if (firstArray[k] + array[l] === 2020) {
        console.log(array[l] * secondArray[k]);
      }
    }
  }
}
