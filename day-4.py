fullinput = """pid:827837505 byr:1976
hgt:187cm
iyr:2016
hcl:#fffffd
eyr:2024
...
def parse_input(some_input):
  inputArray = list(some_input.split("\n\n"))
  return inputArray
    ...
    for x in range(len(list_input)):
        ...
        for field in required_fields:
            for y in range(len(items)):
                if field in items[y]:
                    if field == 'byr':
                        val = int(items[y].replace('byr:', ''))
                        if (val > 1919) and (val < 2003):
                            num_fields = num_fields + 1
                            print('byr valid')
                    if field == 'iyr':
                        val = int(items[y].replace('iyr:', ''))
                        if (val > 2009) and (val < 2021):
                            num_fields = num_fields + 1
                            print('iyr valid')
                    if field == 'eyr':
                        val = int(items[y].replace('eyr:', ''))
                        if (val > 2019) and (val < 2031):
                            num_fields = num_fields + 1
                            print('eyr valid')
                    if field == 'hgt':
                        val = items[y].replace('hgt:', '')
                        list_val = list(val)
                        last_val = list_val[len(list_val)-1]
                        if last_val == 'n':
                            numbers = val.replace('in', '')
                            numbers = int(numbers)
                            #print(numbers)
                            if numbers > 58 and numbers < 77:
                                num_fields = num_fields + 1
                                print('hgt valid')
                        elif last_val == 'm':
                            numbers = val.replace('cm', '')
                            numbers = int(numbers)
                            #print(numbers)
                            if numbers > 149 and numbers < 194:
                                num_fields = num_fields + 1
                                print('hgt valid')
                    if field == 'hcl':
                        val = items[y].replace('hcl:', '')
                        #print(val)
                        flag = 0
                        if len(list(val)) == 7:
                            #print('len')
                            if list(val)[0] == '#':
                                #print('pre-hcl', num_fields)
                                for x in range(1, 7):
                                    #print(list(val)[x])
                                    if list(val)[x] not in hair_color:
                                        print('not in dict')
                                        flag = 1
                                if flag == 0:
                                    num_fields = num_fields + 1
                                    print('hcl valid')
                    if field == 'ecl':
                        val = items[y].replace('ecl:', '')
                        if val in eye_color:
                            num_fields = num_fields + 1
                            print('ecl valid')
                    if field == 'pid':
                        val = items[y].replace('pid:', '')
                        if len(val) == 9:
                            if val.isnumeric():
                                num_fields = num_fields + 1
                                print('pid valid')
