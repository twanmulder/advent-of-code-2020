fullinput = """16-18 h: hhhhhhhhhhhhhhhhhh
...
testinput = """15-16 m: mhmjmzrmmlmmmmmm
3-9 r: trhrrrrrgrrn
1-4 g: ggxz
2-4 z: rsfd
...
input = """8-14 v: pvxlknfvplgktv"""

def part_one(fullinput):
  inputArray = fullinput.splitlines()
  countValid = 0
  for x in range(len(inputArray)):
    countOccurence = 0
    splitInput = inputArray[x].split(" ")
    numbers = splitInput[0]
    letter = splitInput[1].replace(':', '')
    password = list(splitInput[2])

    splitNumbers = numbers.split("-")
    min = int(splitNumbers[0])
    max = int(splitNumbers[1])

    for y in range(len(password)):
      if password[y] == letter:
        countOccurence = countOccurence + 1
    
    if countOccurence >= min:
      if countOccurence <= max:
...
def part_two(some_input):
  ...
  countValid = 0
  for x in range(len(inputArray)):
    split_array = inputArray[x].split()
    countOccurence = 0
    splitInput = inputArray[x].split(" ")
    numbers = splitInput[0]
    letter = splitInput[1].replace(':', '')
    ...
    splitNumbers = numbers.split("-")
    min = int(splitNumbers[0]) - 1
    max = int(splitNumbers[1]) - 1
    ...
    if countOccurence == 1:
      ...
      print("invalid:", countOccurence)

  print("valid passwords:",countValid)
